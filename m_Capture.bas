...
Public Function CaptureClient(frm As Form, Optional x As Integer, Optional y As Integer, Optional x1 As Integer, Optional y1 As Integer) As Picture
'
' Capture the client area of the form.
'
If x1 > 0 Then
With frm
'    Set CaptureClient = CaptureWindow(.hWnd, True, x, y, _
            .ScaleX(x1, .ScaleMode, vbPixels), _
            .ScaleY(y1, .ScaleMode, vbPixels))
    Set CaptureClient = CaptureWindow(frmResize.hWnd, True, x, y, x1, y1)
End With
Else
With frm
